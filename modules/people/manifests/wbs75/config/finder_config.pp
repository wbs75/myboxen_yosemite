class people::wbs75::config::finder_config (
    ...
    property_list_key { 'ComputerViewSettings - CustomViewStyle':
        ensure      =>      present,
        path        =>      "${my_homedir}/Library/Preferences/com.apple.finder.plist",
        key         =>      'clmv',
        value       =>      [ {
                                'dicts'  => 'ComputerViewSettings',
                                'string' => 'CustomViewStyle' }],
        value_type  =>      'array',
    }

    property_list_key { 'ComputerViewSettings - CustomViewStyleVersion':
        ensure      =>      present,
        path        =>      "${my_homedir}/Library/Preferences/com.apple.finder.plist",
        key         =>      1,
        value       =>      [ {
                                'dicts'  => 'ComputerViewSettings',
                                'integer' => 'CustomViewStyleVersion' }],
        value_type  =>      'array',
    }

    property_list_key { 'ComputerViewSettings - GroupBy':
        ensure      =>      present,
        path        =>      "${my_homedir}/Library/Preferences/com.apple.finder.plist",
        key         =>      'Name',
        value       =>      [ {
